...
(() => {
  ...
  const VISION_TOOLS = new Set(["screen_capture", "screenshot-window"]);
  ...
  async function runTool(call) {
    ...
    if (isBlockedTool(name)) {
      if (VISION_TOOLS.has(bareName)) {
        return `ERROR: '${bareName}' is unavailable here - this assistant cannot see images. Do NOT call it again. Inspect the game programmatically instead (e.g. get-instance-properties, get-game-info, search-instances, get-script-content).`;
      }
      return `ERROR: the '${bareName}' command timed out and is unavailable in this environment. Do NOT call it again - complete the task yourself using the other commands (get-data-by-code, search-api, etc.).`;
    }
    // Virtual command: list the MCP server(s) ZeroReal is currently connected
    ...
    if (name === "list_mcp_servers") {
      ...
      const lines = servers.length
        ? servers.map((sv) => {
            const label = sv.id === "real" ? "Real executor (primary)" : `${sv.id} (addon)`;
            return `- ${sv.id}: ${label} - ${sv.alive ? `${sv.tools || 0} commands available` : "offline (no tools)"}`;
          })
        : ["- real: Real executor (primary) - unknown (bridge did not report server health)"];
      return (
        `Output of 'list_mcp_servers':\n` +
        `Connected MCP servers (${lines.length}):\n${lines.join("\n")}\n` +
        `Use list_commands with a "server" param (one of the ids above) to see that server's exact commands. Without "server", list_commands defaults to "real".`
      );
    }
    // Virtual command: list available commands with full details. Defaults to
    // the Real server - a DIFFERENT server's tools only ever show up
    // if the model explicitly asks via {"server": "<id>"} (see list_mcp_servers).
    if (name === "list_commands" || name === "list_tools") {
      await ensureTools();
      const requested = (args.server || "real").trim();
      // The bridge keeps advertising Real's catalogue even with no client
      // attached, so list_commands would hand back the full command list and read
      // as "Real is fine" - then every command silently fails. When Real is
      // actually unusable, short-circuit the DEFAULT (real) listing into a plain
      // "Real is down" note so the model stops instead of firing dead commands.
      if (requested === "real") {
        const s = A.bridge || {};
        const srv = s.servers || [];
        const rbx = srv.find((x) => x.id === "real");
        const rbxAlive = rbx ? !!rbx.alive : (!!s.mcpAlive || srv.some((x) => x.alive));
        const rbxUsable = !!s.connected && rbxAlive && s.studio !== false;
        if (!rbxUsable) {
          const others = srv.filter((x) => x.id !== "real" && x.alive && (x.tools || 0) > 0);
          const otherStr = others.length
            ? `Other connected MCP server(s): ${others.map((x) => x.id).join(", ")}. Call list_mcp_servers, then list_commands with a "server" param to use them for anything that does not need the game.`
            : `No other MCP server is connected right now.`;
          return `Output of '${name}':\nReal is currently OFFLINE (app closed, nothing injected, or its MCP server disabled), so its commands cannot run. This is an environment problem on the user's machine, not your mistake. Tell the user in one short sentence to open Roblox, inject Real and enter a game. ${otherStr}`;
        }
      }
      ...
      // hiding everything.
      const scoped = A.toolList.filter((t) => (t.server || "real") === requested);
      if (!A.toolList.length) return `Output of '${name}':\nNo commands available - the bridge or Real may be offline.`;
      if (!scoped.length) {
        return `Output of '${name}':\nERROR: no server named "${requested}" is connected. Connected servers: ${[...known].join(", ") || "real"}. Call list_mcp_servers to check.`;
      }
    ...
    // Real has no datamodel_type (Studio leftover): drop it if the model writes
    // the JSON form with one, instead of failing the call.
    if (args && typeof args === "object" && "datamodel_type" in args) delete args.datamodel_type;
    const timeout = (name === "get-data-by-code" || name === "eval") ? 30000 : 120000;
    ...
    // The bridge answers SUCCESSFULLY (ok:true) when no client is attached
    // with an explanatory text instead of a result. Surface it as a proper
    // environment ERROR so the model stops and tells the user, instead of
    // treating it as tool output.
    if (r.ok && /Unable to find an active Studio instance|previously active Studio has disconnected|no .*client.*(connected|injected|attached)|bridge not connected/i.test(r.text || "")) {
      ui.banner("warn", "Real client is not connected",
        "Open Roblox, inject Real and enter a game, then try again.");
    ...
    // Real reports Luau parse/runtime problems inside the result text - re-shape
    // those into a real ERROR so the model gets the fix-it hints below and the
    // chip settles red, not green.
    if (r.ok && (bareName === "get-data-by-code" || bareName === "eval") && r.text &&
        /\b(?:ExecuteLuauTool|CommandExecution|Luau):\d+:/.test(r.text)) {
      r = { ok: false, error: r.text.replace(/^(?:\S*(?:ExecuteLuauTool|CommandExecution|Luau):\d+:\s*)+/, "").trim() || r.text };
    }
    if (r.ok) {
    ...
    if (r.kind === "stale-extension") {
      ui.banner("warn", "Reload this page",
        "ZeroReal was updated or reloaded while this tab was open, so this page is running an " +
        "old copy of it and commands can no longer run. Reload the page (F5) to reconnect - your " +
        "bridge and Real are unaffected.");
    ...
    if (r.kind === "timeout") {
      return `ERROR: tool '${name}' timed out after ${(name === "get-data-by-code" || name === "eval") ? 30 : 120}s.\n${r.error}\nTry a shorter/simpler call or check that Real is open with a client injected and responsive.`;
    }
    if (name === "get-data-by-code" || name === "eval") {
      const err = r.error || "";
      const luaCode = args.code || args.expression || "";
      const hint = err.includes("Failed to parse command code")
        ? !String(luaCode).trim()
          ...
          : `Real refused to PARSE the code (${String(luaCode).length} chars sent, so it was not empty). Check the Luau syntax first (use search-api to verify unfamiliar globals); if it looks correct, split the work into several smaller calls.`
        : err.includes("attempt to") || err.includes("nil value")
          ? "Luau runtime error. Check that the API you are calling exists (search-api first). Make sure you return values via 'return' (tables become JSON)."
          : "Check your Luau syntax and verify every global against search-api before retrying.";
      return `ERROR in ${name}: ${err}\n\n${hint}\n\nFix the code and retry.`;
    }
  ...
  function argSummary(call) {
    if (!call) return "";
    if (call.tool === "get-data-by-code" || call.tool === "execute_luau") {
      const code = (call.arguments && (call.arguments.code || call.arguments.expression)) || "";
  ...
  function callBody(call) {
    const a = call.arguments || {};
    if (call.tool === "get-data-by-code" || call.tool === "execute_luau") return (a.code || a.expression || "").trim();
        ...
        if (res.kind === "tool") {
          ...
          if (!isErr && call.tool !== "list_commands" && A.toolList.length) {
            ...
            if (A.toolCallsSinceReminder >= REMIND_TOOLS_EVERY) {
              ...
              // Scope the reminder to the primary Real server, exactly like
              // list_commands: re-injecting EVERY connected server's tools
              // merged flat would bloat the model's context. Anti-drift only
              // needs the primary Real set.
              const roblox = A.toolList.filter((t) => (t.server || "real") === "real");
  ...
  function withSysResend(text) {
    ...
    const rider =
      "\n\n────────────────────────────────\n" +
      ZS.RESEND_MARKER + "\n" +
      "(System note from ZeroReal - an automatic re-statement of your operating " +
      ...
      "commands. To be explicit: the ZeroReal extension IS running in this page right " +
  ...
  async function startSession() {
    ...
    try {
      ...
      if (!A.toolList.length) {
        ui.banner("warn", "Bridge or Real offline",
          "Could not fetch Real tools. Run start-real.bat and make sure Real is open with a client injected, then try again.");
        return;
      }
      ...
      if (startRes.kind === "tool" && startRes.calls && startRes.calls.length === 1 &&
        ...
        // Real down short-circuits list_commands into a plain "offline" note
        ...
        // (the bridge may still advertise a cached catalogue even with no client
        // attached, so A.toolList still has entries that were never actually
        // usable this boot).
        if (/Real is currently OFFLINE/.test(toolFeedback)) {
          decorate.toolBox(startRes.item, "Loading commands", "err", "Real offline", true);
        } else {
          // Count what the model ACTUALLY received: list_commands is scoped to the
          // primary Real server, so showing A.toolList.length (every
          // connected server merged) overstated the boot count. Count the
          // Real-scoped tools instead, matching the real result.
          const robloxCount = A.toolList.filter((t) => (t.server || "real") === "real").length;
      ...
      ui.setStarted(true);
      ui.toast(`Agent ready. Ask ${P.displayName} to do something in the game.`);
    } catch (e) {
  ...
  const ui = (() => {
    ...
    function build() {
      ...
      root.innerHTML = `
        <div id="zs-bar">
          ...
          <span id="zs-brand">ZeroReal <span class="zs-free">v${EXT_VERSION}</span></span>
    ...
    function mergedMcpServers() {
      const live = ((A.bridge && A.bridge.servers) || []).filter((sv) => sv.id !== "real");
    ...
    // Derive a config-safe server id from a display name (real is reserved).
    function mcpSlug(name) {
      let s = String(name || "").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "");
      if (!s || s === "real") s = `addon-${s || "server"}`;
      let id = s, n = 2;
    ...
    function buildMenu() {
      ...
      // Real heads the list - greyed out, no health dot (its own status
      // is already the main ZeroReal dot elsewhere) and no remove button (it's
      // the primary server, protected bridge-side too).
      let mcpList =
        `<div class="zs-mcp-item zs-mcp-item-primary"><div class="zs-mcp-info"><span class="zs-mcp-name">Real</span><span class="zs-mcp-url">primary - always connected</span></div></div>`;
      ...
      menuEl.innerHTML =
        `<div class="zs-menu-head"><span class="zs-menu-logo">ZeroReal</span><span class="zs-menu-tag">v${EXT_VERSION}</span></div>
         ...
         <section class="zs-menu-sec">
           <div class="zs-sec-label"><span>MCP servers</span></div>
            <div class="zs-menu-note">Real is always connected (primary). Addon servers are experimental and managed bridge-side.</div>
           ${mcpList}
    ...
    function buildSetup() {
      ...
      const videoBtn = VIDEO_URL
        ...
        : "";
      setupCard.innerHTML =
        `<div id="zs-setup-head"><span id="zs-setup-logo">ZeroReal</span><span id="zs-setup-tag">Setup</span></div>` +
        `<div id="zs-setup-sub">The <b>Bridge</b> is what connects this chat to Real. Three steps and you're running.</div>` +
        `<ol id="zs-setup-steps">` +
          `<li>Keep the Real app open (MCP server on)</li>` +
          `<li>Run <code>start-real.bat</code></li>` +
          `<li>Back here, click <b>Start Real agent</b></li>` +
        `</ol>` +
    ...
    function renderBar() {
      ...
      // chats share a key, and the conversation id only appears mid-bootstrap).
        ...
        if (A.bridge && A.bridge.connected === false) {
          ...
          toneClass = "warn"; warn = true;
          msg = `<b>Agent active</b> · bridge offline, run start-real.bat`;
        } else if ((placeDown || appDown || studioDown) && addonOk) {
          // DEGRADED session by CHOICE: the user started the agent with Real
          // down but other MCP server(s) alive - they may only want the addon
          // tools. Keep the YELLOW dot as the honest health signal, but do NOT
          // keep the red imperative nag on screen for the whole session
          // (warn=false → no zs-state-warn red text).
          toneClass = "warn";
          msg = `<b>Agent active</b>${tools ? ` · ${tools} tools` : ""} · Real offline`;
        } else if (placeDown) {
          toneClass = "warn"; warn = true;
          msg = `<b>Agent active</b> · enter a game with an injected client`;
        } else if (appDown || studioDown) {
          toneClass = "warn"; warn = true;
          msg = `<b>Agent active</b> · open Roblox, inject Real & enter a game`;
        } else {
          toneClass = "active";
        ...
        if (bridgeOk) {
          ...
          label = "▶︎ Start Real agent"; kind = "start";
        } else if (addonOk) {
          // Real is down but another MCP server is live: allow a DEGRADED start
          // (yellow). The agent runs on the other server(s); Real tools stay
          // unavailable until a client is back. Button enabled, but visibly warned.
          toneClass = "warn"; warn = true;
          msg = !A.bridge.connected
            ? `Run <b>start-real.bat</b> on your PC.`
            : `<b>Real offline</b> - start with your other MCP server(s).`;
          label = "▶︎ Start agent (Real offline)"; kind = "start-degraded";
        } else {
          toneClass = "warn"; warn = true;
          msg = !A.bridge.connected
            ? `Run <b>start-real.bat</b> on your PC.`
            : (appDown || studioDown)
              ? `Open <b>Roblox</b>, inject <b>Real</b> &amp; enter a game.`
              : `Inject <b>Real</b> in a game for the tools.`;
          label = "▶︎ Start Real agent"; kind = "start";
        }
    ...
    function setStatus(s) {
      ...
      // ZeroReal status tracks ONLY the primary Real MCP server. Every other
      // server is an addon and must NEVER make the dot/gate look connected while
      // Real itself is down. Old bridges don't send per-server health, so fall
      // back to the aggregate signals they do send (mcpAlive / total tools).
      const roblox = servers.find((x) => x.id === "real");
      const mcpUp = roblox ? !!roblox.alive : (!!s.mcpAlive || servers.some((x) => x.alive));
      // Real-only count drives the connectivity gate (the dot must never look
      // green off an addon while Real itself is down)...
      const robloxTools = roblox ? (roblox.tools || 0) : (s.tools || 0);
      const mcpOk = s.connected && (mcpUp || robloxTools > 0);
      // ...but the DISPLAYED count is the aggregate across every server, so it
      // stays consistent with the bar and doesn't under-report when addon
      // servers are loaded.
      const totalTools = servers.reduce((n, x) => n + (x.tools || 0), 0) || s.tools || robloxTools;
      // studio === false means the MCP server answered but no client is USABLE
      // (Real open but nothing injected / not in a game).
      // null/undefined = unknown (probe busy) → don't degrade.
      ...
      // studioProc: whether a Roblox process exists (bridge-side check, may be
      // null on this bridge). Kept only as a display hint.
      const procUp = s.studioProc === true;
      let txt;
      if (!s.connected) txt = "Bridge offline, run start-real.bat";
      else if (!mcpOk) txt = "Bridge OK, open Real";
      else if (noPlace) txt = "Real is up but no game client injected - enter a game";
      else if (noApp) txt = procUp
        ? "Roblox open but Real not injected - inject Real and enter a game"
        : "Real not connected - inject Real in a game";
      else if (studioOff) txt = "Real client not connected - inject Real in a game";
      ...
      appDown = noApp;
      studioProcUp = procUp;
      // A non-Real MCP that is actually alive. When
      // Real itself is down but such a server is present, the session can still
      // start in a DEGRADED mode - the agent just can't touch the game until
      // a client is back. Gated on s.connected so a dropped bridge never reads as usable.
      addonOk = !!s.connected && servers.some((x) => x.id !== "real" && x.alive && (x.tools || 0) > 0);
    ...
    function staleExtensionAlert() {
      ...
      b.innerHTML = `<div class="zs-banner-t">⚠ Reload this page to reconnect ZeroReal</div>
        <div class="zs-banner-m">ZeroReal was updated or reloaded while this tab was open, so this page is still running the old copy and commands can no longer run. Your bridge and Real are fine - only this page needs refreshing.</div>
    ...
    function bridgeAlert(on) {
      ...
      const videoLink = VIDEO_URL
        ...
        : "";
      b.innerHTML = `<div class="zs-banner-t">⚠ Lost connection to ZeroReal</div>
        <div class="zs-banner-m">The ZeroReal bridge stopped on your PC. Restart it (run start-real.bat and keep Real open with a client injected): the agent will reconnect automatically as soon as it is detected again.</div>
    ...
    function nudgeStart() {
      ...
      if (!nudged) {
        nudged = true;
        toast("Tip: click “▶︎ Start Real agent” to let the AI control the game via Real.");
      }
      if (!actionBtn) return;
