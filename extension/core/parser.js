...
// core/parser.js - ZeroReal command parsing. PURE string logic, zero DOM:
...
const ZSParse = (() => {
  ...
  // A trailing `:Server` / `:Client` suffix is accepted but IGNORED - it is a
  // leftover from the Studio build (Real has no datamodel_type; code always runs
  // in the injected client). The bridge also maps the legacy name execute_luau.
  const LUA_START_RE = /###\s*lua(?:\s*[:\-_ ]\s*(edit|client|server))?\s*(?:###|---)/i;
  const LUA_END_RE = /###\s*end[_\- ]?lua\s*###/i;
  const LUA_TOOL = "get-data-by-code";

  // Find the first LUA start marker at or after `from`. Returns { pos, len }
  ...
  function parseToolCalls(r) {
    ...
    while (true) {
      ...
      if (ls !== -1 && le !== -1 && le > ls) {
        out.push({ tool: LUA_TOOL, arguments: { code: stripCodeChrome(body.slice(ls + luaLen, le).trim()) } });
    ...
    if (out.length === 0) {
      ...
      if (ls !== -1 && le !== -1 && le > ls) {
        out.push({ tool: LUA_TOOL, arguments: { code: stripCodeChrome(r.slice(ls + luaLen, le).trim()) } });
      }
    }
    return out.map(cleanLuaCall);
  }

  // Some models wrap Luau in the JSON envelope AND keep the ###LUA### /
  // ###END_LUA### markers INSIDE the code string, e.g.
  //   {"command":"get-data-by-code","params":{"code":"###LUA###\n<lua>\n###END_LUA###"}}
  // Once JSON-decoded the code still starts with the literal markers, which the
  // tool rejects. Strip a leading start marker and a trailing end marker from
  // the code. Accepts the legacy execute_luau name too (bridge alias). No-op
  // for a clean code string.
  function cleanLuaCall(call) {
    if (!call || (call.tool !== LUA_TOOL && call.tool !== "execute_luau")) return call;
    if (call.tool === "execute_luau") call.tool = LUA_TOOL;
    ...
    delete call.arguments.datamodel_type; // Real has no datamodel (Studio leftover)
    return call;
  }
  ...
  function toolNameFromText(txt) {
    ...
    if (txt.includes("get-data-by-code") || txt.includes("execute_luau") || LUA_START_RE.test(txt)) return "get-data-by-code";
    return "command";
  }
