...
function render(s) {
  ...
  const studioOff = mcpOk && s.studio === false; // MCP up but no Real client attached
  const ok = mcpOk && !studioOff;
  dot.className = "dot " + (s.connected ? (ok ? "on" : "warn") : "");
  state.textContent = s.connected
    ? (ok ? "Connected · Real ready"
        : studioOff ? "Real not connected · inject Real in a game"
        : "Bridge OK · open Real and inject")
    : "Bridge offline";
  tools.textContent = s.connected ? `${s.tools || 0} tools available` : "Run start-real.bat";
...
document.getElementById("restart").addEventListener("click", (e) => {
  ...
  chrome.runtime.sendMessage({ type: "restart_mcp" }, () => {
    e.target.textContent = "⟳ Refresh Real tools";
