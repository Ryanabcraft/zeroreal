...
//
// DeepSeek notes (validated live 2026-09 on the unified-model UI):
...
//  - The Instant/Expert/Vision model tabs are GONE (unified model, 2026-09):
//    the composer now has two .ds-toggle-button pills - "Pensamento Profundo"
//    (thinking, ON by default) and "Pesquisa inteligente" (search, ON by default
//    on fresh chats - the agent keeps BOTH on at startup). No radiogroup exists.
//  - The send button is a div[role=button].ds-button--primary. Disabled state is
//    the `ds-button--disabled` CLASS (no aria-disabled attr). While streaming it
//    shows a STOP rounded square (<path> starting "M2", e.g. "M2 4.88C2…") and
//    when idle a SEND arrow (<path> starting "M8…"); see isStopBtn().
//    During the reasoning phase there is NO stop button - only text growth
//    says "still alive".
//  - The unified model includes image understanding (ex-Vision), so
//    supportsVision is unconditionally true.
...
const ZSProvider = (() => {
  ...
  const S = {
    ...
    // Disabled-send marker. The button is a div[role=button] with NO
    // aria-disabled attribute - DeepSeek signals "can't send" with this class.
    sendDisabledCls: "ds-button--disabled",
  ...
  // Error / state regexes (English + French + Portuguese + Spanish - DeepSeek's
  // UI follows the locale; the unified-model UI was validated in pt-BR).
  const RE = {
    contextLimit: new RegExp(
      [
        "conversation.{0,20}(too long|trop long|muito longa|longa demais)",
    ...
    tooLong: /conversation .{0,20}(too long|getting too long|trop longue|muito longa)/i,
    busy: /server is busy|serveur est occup|please try again|réessayer plus tard|system is currently busy/i,
    continueBtn: /^(continue|continuer|continuar|继续(生成)?|fortfahren|seguir|続行)$/i,
    stopped: /(arrêté|arrété|stopped|interrompid|已停止|停止生成|已暂停)/i,
    expertMode: /expert|专家|专业/i,
    visionMode: /vision|视觉|图像|多模态/i,
    deepThink: /pensée profonde|pensee profonde|profonde|pensamento profundo|pensamiento profundo|réflexion|reflexion|deep ?think|深度思考|r1/i,
    searchMode: /recherche intelligente|smart search|pesquisa inteligente|búsqueda|search|web|搜索/i,
  };
  ...
  // A genuinely FRESH/new chat. The old UI showed an Expert/Instant/Vision mode
  // radiogroup only on brand-new empty chats; the unified-model UI has no
  // radiogroup at all, so emptiness is the whole signal now.
  const isFreshChat = () => chatIsEmpty();

  // The whole composer "box" = the smallest ancestor that contains the input
  // and the send button. The core's Start gate hides this entire frame at once.
  ...
  // ancestor of the textarea that also holds the send button. It is a
  // vertical flow container (textarea + the toggle pill row), so adding
  ...
  // ── Composer mode: thinking ON, Search ON ───────────────────────────────
  // Driven once at session start only; the user can switch afterwards.
  // Unified-model UI (2026-09): no model tabs anymore. The agent runs with
  // EVERYTHING on: "Pensamento Profundo" (thinking) ON and "Pesquisa
  // inteligente" (search) ON. Both are .ds-toggle-button pills with
  // aria-pressed + the ds-toggle-button--selected class (see
  // isPressedOn/isPressedOff).
  ...
  function enforceComposer(reason) {
    ...
    // to toggle thinking/search afterwards without ZeroReal reverting it.
    if (!reason) return composerModeState();
    try {
      // Thinking ON (Pensamento Profundo): the agent's reasoning model. If the
      // user deliberately turned it off, the startup pass turns it back on once;
      // afterwards it is theirs to control.
      const deepThink = findToggleBy(RE.deepThink);
      if (deepThink && isPressedOff(deepThink) && toggleGuard("dt")) {
        try { deepThink.click(); } catch (e) { diag("mode_fallback", { reason, target: "deepThink", error: String(e && e.message || e) }); }
      }

      // Search ON as well (user choice: everything activated). If it is off,
      // turn it on once; afterwards it is theirs to control.
      const search = findToggleBy(RE.searchMode);
      if (search && isPressedOff(search) && toggleGuard("s")) {
  ...
  // Toggle clicks flip asynchronously (React state): a re-read in the same tick
  // still shows the OLD state, and the ensure loop would click AGAIN - flipping
  // the toggle right back (validated live 2026-09: double-click re-enabled
  // Search). One click per toggle per ~1s max; the loop's sleep covers the rest.
  const _toggleAt = {};
  function toggleGuard(which) {
    const now = Date.now();
    if (now - (_toggleAt[which] || 0) < 1000) return false;
    _toggleAt[which] = now;
    return true;
  }

  // Drive the composer into its required modes; returns the final state with
  // `.ready` (the core gates session start on it). Ready = thinking on (or no
  // thinking toggle present at all) and search on (or no search toggle at
  // all). No model radios exist in the unified UI, so they play no part.
  async function ensureComposerReady(reason) {
    let state = composerModeState();
    for (let i = 0; i < 12; i++) {
      state = enforceComposer(reason);
      await sleep(350); // let React settle the toggle flip before re-reading
      state = composerModeState();
      const thinkOk = state.deepThinkOn || !state.deepThinkFound;
      const searchOk = !state.searchFound || !state.searchOff; // absent toggle = nothing to enforce
      if (thinkOk && searchOk) break;
    }
    state = composerModeState();
    const ready = (state.deepThinkOn || !state.deepThinkFound) &&
      (!state.searchFound || !state.searchOff);
    diag("mode_ready", { reason, ...state });
    return { ...state, ready };
  }

  // DeepSeek's footer button doubles as SEND (an upward arrow) and STOP (a
  // filled rounded square). Both are drawn as a <path>: the send arrow starts
  // mid-glyph ("M8.31…", validated live 2026-09), the stop square starts at a
  // corner near the origin ("M2 4.88C2…", validated live 2026-09). We treat the
  // button as "stop" when it carries a <rect> (legacy) OR a square-ish path
  // (leading move to x ≤ 3) - never the M8 arrow. One-liner to update if
  // DeepSeek reskins the footer button.
  ...
  // Send-allowed gate. The button is a div[role=button] with NO aria-disabled
  // attribute - DeepSeek signals "can't send" (empty composer, uploading,
  // offline) with the ds-button--disabled CLASS (validated live 2026-09: the
  // old aria-disabled-only check let clicks through on a disabled button and
  // silently dropped the message). Never true on the stop square.
  function btnEnabled(btn) {
    if (!btn || isStopBtn(btn)) return false;
    if (btn.getAttribute("aria-disabled") === "true") return false;
    if (btn.classList && btn.classList.contains(S.sendDisabledCls)) return false;
    return true;
  }
  ...
  // or the button is disabled (ds-button--disabled class - see btnEnabled).
  ...
  async function typeAndSend(text, images) {
    ...
    await waitFor(() => btnEnabled(document.querySelector(S.sendBtn)), 800);
  ...
  return {
    ...
    // The unified model (2026-09) merged Instant/Expert/Vision, including image
    // understanding - so image tools are always allowed. The old per-tab latch
    // (detectVision/badgeVision) is kept below for reference but no longer gates.
